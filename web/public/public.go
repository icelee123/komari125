...
func Static(r *gin.RouterGroup, noRoute func(handlers ...gin.HandlerFunc)) {
	...
	adminThemeFS, err := fs.Sub(AdminFS, "adminTheme")
	if err != nil {
		panic("you may forget to put dist of the official frontend to web/public/adminTheme/dist")
	}
	...
	getAdminFileContent := func(relativePath string) ([]byte, string, bool) {
		cleanPath := strings.TrimPrefix(relativePath, "/")
		cleanPath = filepath.Clean(cleanPath)
		embedPath := filepath.ToSlash(cleanPath)
		...
		if content, err := fs.ReadFile(adminThemeFS, embedPath); err == nil {
			return content, mime.TypeByExtension(filepath.Ext(embedPath)), true
		}
	...
	serveIndex := func(c *gin.Context) {
		...
		// 管理端使用 Komari 官方内置前端，不跟随展示主题。
		if strings.HasPrefix(reqPath, "/admin") {
			content, _, exists := getAdminFileContent(path.Join(DistDir, IndexFile))
			if !exists {
				c.String(http.StatusNotFound, "Index file missing (checked adminTheme/dist/index.html).")
				return
			}
	...
	noRoute(func(c *gin.Context) {
		...
		// 官方管理端使用根路径 /assets、/manifest.json 等资源。
		if content, mimeType, exists := getAdminFileContent(path.Join(DistDir, reqPath)); exists {
			c.Data(http.StatusOK, mimeType, content)
			return
		}
