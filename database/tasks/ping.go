...
func AddPingTask(clients []string, defaultOn bool, name string, target, task_type string, interval int) (uint, error) {
	if interval < models.MinPingIntervalSeconds {
		return 0, fmt.Errorf("ping interval must be at least %d seconds", models.MinPingIntervalSeconds)
	}
...
func EditPingTask(tasks []*models.PingTask) error {
	...
	for _, task := range tasks {
		if task.Interval < models.MinPingIntervalSeconds {
			return fmt.Errorf("ping interval must be at least %d seconds", models.MinPingIntervalSeconds)
		}
